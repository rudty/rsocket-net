...
public class SocketTransport : MpscMessageLooper<FrameBuffer>, IRSocketTransport
{
...
#pragma warning restore CS0649
	...
	//public static async ValueTask<SequencePosition> SendAsync(Socket socket, ReadOnlySequence<byte> buffer, SequencePosition position, SocketFlags socketFlags, CancellationToken cancellationToken = default)
	//{
	...
	public override async ValueTask OnItemReceived(FrameBuffer frame)
	{
		try
		{
			await Socket.SendAsync(frame.ReadOnlyMemory, SocketFlags.None);
		}
		finally
		{
			frame.Release();
